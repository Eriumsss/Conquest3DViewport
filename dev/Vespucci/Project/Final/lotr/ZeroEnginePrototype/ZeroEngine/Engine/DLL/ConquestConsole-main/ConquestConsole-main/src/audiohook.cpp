...
static DWORD __cdecl HookedPostEvent(DWORD eventId, DWORD gameObjId, DWORD flags, void* callback, void* cookie) {
    ...
    // ========================================================================
    // Custom Audio Override Check
    // Try to play custom audio from ConquestMods/audio/ folder
    // Also handles stop events to stop our custom audio
    // ========================================================================
    if (CustomAudio::IsInitialized()) {
        // Look up event name from mapping
        auto mappingIt = g_EventBankMapping.find(eventId);
        if (mappingIt != g_EventBankMapping.end()) {
            const char* eventName = mappingIt->second.c_str();
            // Only try if we have a real event name (not hex fallback)
            if (eventName && eventName[0] != '0') {
                // First check if this is a stop event - need to stop our custom audio
                if (CustomAudio::HandleStopEvent(eventName)) {
                    if (g_LoggingEnabled) {
                        char buf[256];
                        sprintf_s(buf, "[AudioHook] Custom audio stop handled: %s (0x%08X)\n", eventName, eventId);
                        OutputDebugStringA(buf);
                    }
                }

                // Then try to play custom audio if this is a play event
                if (CustomAudio::TryPlayCustomAudio(eventName)) {
                    customAudioPlayed = true;
                    // Log that we overrode this event
                    if (g_LoggingEnabled) {
                        char buf[256];
                        sprintf_s(buf, "[AudioHook] Custom audio override: %s (0x%08X)\n", eventName, eventId);
                        OutputDebugStringA(buf);
                    }
                }
            }
    ...
    // If custom audio played, skip original Wwise call but still log the event
...
bool Initialize() {
    ...
    // Initialize separate Wwise engine instance for custom audio playback
    HWND hwnd = NULL;
    IDirect3DDevice9* device = D3D9Hook::GetDevice();
    if (device) {
    ...
    if (!WwiseSDK::Initialize(hwnd)) {
        OutputDebugStringA("[AudioHook] Warning: WwiseSDK initialization failed\n");
    }

    // Initialize custom audio tracking layer (auto-loads banks from ConquestMods/audio/banks/)
    if (!CustomAudio::Initialize()) {
        OutputDebugStringA("[AudioHook] Warning: CustomAudio initialization failed\n");
    }
...
void Shutdown() {
    RemoveHooks();
    CustomAudio::Shutdown();  // Shutdown custom audio tracking first
    WwiseSDK::Shutdown();     // Terminate separate Wwise engine instance
    g_EventNames.clear();
