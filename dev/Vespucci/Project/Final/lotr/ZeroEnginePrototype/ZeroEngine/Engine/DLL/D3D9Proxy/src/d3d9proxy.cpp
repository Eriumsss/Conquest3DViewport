...
// Flag to ensure patches are only applied once
static bool g_PatchesApplied = false;

// Exported Direct3DCreate9
extern "C" IDirect3D9* WINAPI Direct3DCreate9(UINT SDKVersion) {
    // Apply memory patches on first D3D call (safer than DllMain)
    if (!g_PatchesApplied) {
...
BOOL APIENTRY DllMain(HMODULE hModule, DWORD reason, LPVOID lpReserved) {
    switch (reason) {
        case DLL_PROCESS_ATTACH:
            ...
            // Don't patch here - too early, loader lock held
            break;
        case DLL_PROCESS_DETACH:
            CleanupMemoryPatcher();
