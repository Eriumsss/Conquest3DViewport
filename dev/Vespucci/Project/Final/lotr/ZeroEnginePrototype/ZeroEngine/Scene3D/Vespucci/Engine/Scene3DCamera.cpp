...
// ---------------------------------------------------------------------------
// Viewport input gate — closes the "WASD still moves the camera when I'm
// typing in a side panel" hole. Scene3DCamera polls GetAsyncKeyState
// directly, which bypasses the WM_KEYDOWN g_uiWantCaptureKeyboard guard
// in WindowProc (that guard only suppresses the g_keys[] table the
// camera doesn't read). Host pushes the current capture state in once
// per frame from RenderFrame and we early-bail every WASD/Space/C read
// when ImGui owns the keyboard OR the cursor isn't over the viewport.
// Mouse-button-down is treated as "user is actively driving the camera"
// so they don't get yanked out of a fly-around if the cursor scrubs
// across a docked panel mid-drag.
// ---------------------------------------------------------------------------
static bool s_camGate_uiBlocksKeys = false;
static bool s_camGate_cursorInVP   = true;
static bool s_camGate_mouseHeldVP  = false;

extern "C" void Scene3D_SetCameraInputGate(int uiKb, int cursorInVP, int mouseHeldFromVP) {
    s_camGate_uiBlocksKeys = (uiKb != 0);
    s_camGate_cursorInVP   = (cursorInVP != 0);
    s_camGate_mouseHeldVP  = (mouseHeldFromVP != 0);
}

static inline bool CameraKeysAllowed() {
    if (s_camGate_uiBlocksKeys) return false;
    return s_camGate_cursorInVP || s_camGate_mouseHeldVP;
}
                                  ...
                                  float deltaTime)
{
    ...
    const bool keysOK = CameraKeysAllowed();
    if (keysOK && (GetAsyncKeyState('W') & 0x8000)) inputUD += 1.0f;
    if (keysOK && (GetAsyncKeyState('S') & 0x8000)) inputUD -= 1.0f;
    if (keysOK && (GetAsyncKeyState('A') & 0x8000)) inputLR -= 1.0f;
    if (keysOK && (GetAsyncKeyState('D') & 0x8000)) inputLR += 1.0f;

    hkBool wantJump = keysOK && ((GetAsyncKeyState(VK_SPACE) & 0x8000) != 0);
...
void Scene3DRenderer::updateCameraFromInput(float deltaTime)
{
    ...
    // ── Cinematic override — skip all input, just apply orbit params to hkgCamera ──
    if (m_cineOverride) {
        ...
        float posArray[3] = { m_cameraTarget(0) + x, m_cameraTarget(1) + y, m_cameraTarget(2) + z };
    ...
    if (m_flyCameraActive)
    {
        if (m_physicsActive && m_physWorld && m_physCharProxy && m_physCharCtx)
        {
            ...
            // ── Update player animation state (idle/walk/run) ──
            {
                ...
                const bool keysOKAnim = CameraKeysAllowed();
                if (keysOKAnim && (GetAsyncKeyState('W') & 0x8000)) inputUD += 1.0f;
                if (keysOKAnim && (GetAsyncKeyState('S') & 0x8000)) inputUD -= 1.0f;
                if (keysOKAnim && (GetAsyncKeyState('A') & 0x8000)) inputLR -= 1.0f;
                if (keysOKAnim && (GetAsyncKeyState('D') & 0x8000)) inputLR += 1.0f;
    ...
    if (deltaTime > 0.0f && CameraKeysAllowed())
    {
...
void Scene3DRenderer::focusSelection()
{
        ...
        m_selectedBoneIndex < m_gameModel->skeleton->m_numBones)
    {
        ...
        // Same goddamn fix as autoFrameLoadedModel: focus on the hand
        // bone where the hand ACTUALLY is right now, not its T-pose
        // memorial location ten feet to the left of the actual fucking
        // hand. Live pose if we have a clip, reference as fallback.
        if (m_jsonAnim)
...
void Scene3DRenderer::autoFrameLoadedModel()
{
    ...
    {
        hkaPose framePose(m_gameModel->skeleton);
        // Frame what is actually on the goddamn screen, not what was on
        // the screen in 1998 when somebody authored the bind pose. The
        // old code always grabbed the T-pose bounds, so the user could
        // be watching a character KNEELING or JUMPING or motherfucking
        // DEAD, hit "frame selected", and the camera would dutifully
        // park itself staring at empty air half a meter above the
        // actual mesh because that's where the T-pose elbows USED to
        // be. Pure rookie-tier garbage, classic case of a programmer
        // who never opened the editor and watched a real animation
        // play. Sample the live clip if we've got one. Reference pose
        // is the FALLBACK, not the answer.
        if (m_jsonAnim)
        {
            buildPoseFromJson(framePose, m_jsonAnimTime);
        }
        else
        {
            framePose.setToReferencePose();
        }
        framePose.syncModelSpace();
        boneMatrices.resize(boneCount * 16);
        for (int b = 0; b < boneCount; ++b)
        {
            framePose.getBoneModelSpace(b).get4x4ColumnMajor(&boneMatrices[b * 16]);
        }
    }
