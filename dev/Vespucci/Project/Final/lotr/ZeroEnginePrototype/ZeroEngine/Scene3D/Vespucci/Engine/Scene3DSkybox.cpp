...
// Global sky tint — set by LevelScene after lighting extraction
// Applies inscatter atmospheric color to the skybox texture
float g_skyTint[4] = { 1.0f, 1.0f, 1.0f, 1.0f };
...
struct SkyboxD3DState
{
    ...
    SkyboxD3DState()
        ...
        , lighting(FALSE)
        , colorOp0(D3DTOP_MODULATE)
        , colorArg10(D3DTA_TEXTURE)
        , colorArg20(D3DTA_DIFFUSE)
        , alphaOp0(D3DTOP_MODULATE)
        , alphaArg10(D3DTA_TEXTURE)
        , alphaArg20(D3DTA_DIFFUSE)
        , colorOp1(D3DTOP_DISABLE)
        , alphaOp1(D3DTOP_DISABLE)
        , tex0(NULL)
    {
    }
    ...
    void capture(IDirect3DDevice9* dev)
    {
        ...
        device->GetTextureStageState(0, D3DTSS_COLORARG1, &colorArg10);
        device->GetTextureStageState(0, D3DTSS_COLORARG2, &colorArg20);
        device->GetTextureStageState(0, D3DTSS_ALPHAOP,   &alphaOp0);
        device->GetTextureStageState(0, D3DTSS_ALPHAARG1, &alphaArg10);
        device->GetTextureStageState(0, D3DTSS_ALPHAARG2, &alphaArg20);
        device->GetTextureStageState(1, D3DTSS_COLOROP,   &colorOp1);
        device->GetTextureStageState(1, D3DTSS_ALPHAOP,   &alphaOp1);
    ...
    void restore()
    {
        ...
        device->SetTextureStageState(0, D3DTSS_COLOROP,   colorOp0);
        device->SetTextureStageState(0, D3DTSS_COLORARG1, colorArg10);
        device->SetTextureStageState(0, D3DTSS_COLORARG2, colorArg20);
        device->SetTextureStageState(0, D3DTSS_ALPHAOP,   alphaOp0);
        device->SetTextureStageState(0, D3DTSS_ALPHAARG1, alphaArg10);
        device->SetTextureStageState(0, D3DTSS_ALPHAARG2, alphaArg20);
        device->SetTextureStageState(1, D3DTSS_COLOROP,   colorOp1);
        device->SetTextureStageState(1, D3DTSS_ALPHAOP,   alphaOp1);
        captured = false;
    }
...
void Scene3DRenderer::renderSkyboxMesh()
{
    ...
    SkyboxD3DState d3dState;
    d3dState.capture(m_d3dDevice);
    ...
    int backdropW = m_windowWidth;
    int backdropH = m_windowHeight;
    if (m_d3dDevice)
    {
        D3DVIEWPORT9 activeVP;
        if (SUCCEEDED(m_d3dDevice->GetViewport(&activeVP)) &&
            activeVP.Width > 0 && activeVP.Height > 0)
        {
            backdropW = (int)activeVP.Width;
            backdropH = (int)activeVP.Height;
        }
    }

    bool drewBackdrop = false;
    if (drawBackdrop && primaryTexture && backdropW > 0 && backdropH > 0)
    {
        float cx, cy, cz;
        float fx, fy, fz;
        float rx, ry, rz;
        float ux, uy, uz;
        bool haveCameraBasis = false;
        ...
        if (m_camera)
        {
            ...
            float camRight[3];
            float camUp[3];
            m_camera->getFrom(camFrom);
            m_camera->getRight(camRight);
            m_camera->getUp(camUp);

            cx = camFrom[0]; cy = camFrom[1]; cz = camFrom[2];
            rx = camRight[0]; ry = camRight[1]; rz = camRight[2];
            ux = camUp[0]; uy = camUp[1]; uz = camUp[2];

            float rl = ZLength3f(rx, ry, rz);
            float ul = ZLength3f(ux, uy, uz);
            if (rl > 1e-5f && ul > 1e-5f)
            {
                ZNormalize3f(rx, ry, rz);
                ZNormalize3f(ux, uy, uz);
                ZCross3f(fx, fy, fz, ux, uy, uz, rx, ry, rz);
                if (ZLength3f(fx, fy, fz) > 1e-5f)
                {
                    ZNormalize3f(fx, fy, fz);
                    ZCross3f(ux, uy, uz, rx, ry, rz, fx, fy, fz);
                    if (ZLength3f(ux, uy, uz) > 1e-5f)
                    {
                        ZNormalize3f(ux, uy, uz);
                        haveCameraBasis = true;
                    }
                }
            }
        }
        ...
        if (!haveCameraBasis)
        {
            // THE SKY MUST FOLLOW THE REAL CAMERA, NOT SOME STALE ORBIT
            // NUMBERS LEFT IN A DRAWER. This fallback only exists for the
            // pathological case where Havok hands us a dead basis. If this
            // path is hot, the camera is already fucked and the backdrop is
            // just the smoke alarm screaming before the graphics go nuclear.
            cx = m_cameraDistance * cosf(m_cameraPitch) * sinf(m_cameraYaw) + m_cameraTarget(0);
            cy = m_cameraDistance * sinf(m_cameraPitch) + m_cameraTarget(1);
            cz = m_cameraDistance * cosf(m_cameraPitch) * cosf(m_cameraYaw) + m_cameraTarget(2);
        ...
        if (fl > 1e-5f)
        {
            ...
            float aspect = (float)backdropW / (float)backdropH;
            ...
            float white[4] = { g_skyTint[0], g_skyTint[1], g_skyTint[2], g_skyTint[3] };
    ...
    for (int i = 0; i < model->meshPartCount; i++)
    {
        ...
        float white[] = { g_skyTint[0], g_skyTint[1], g_skyTint[2], g_skyTint[3] };
        m_context->setCurrentColor4(white);
        m_context->beginGroup(HKG_IMM_TRIANGLE_LIST);
    ...
    static int s_lastDrewBackdrop = -1;
    ...
    if (s_lastSkyboxIndex != m_activeSkyboxIndex ||
        ...
        s_lastDrewBackdrop != (drewBackdrop ? 1 : 0) ||
    ...
    {
        char msg[320];
        sprintf_s(msg, "Skybox '%s' mode=%s: backdrop=%d rendered=%d skippedMode=%d skippedNoTexture=%d skippedNoUv=%d hasNoUv=%d",
            getActiveSkyboxName(), getSkyRenderModeName(), drewBackdrop ? 1 : 0,
            renderedParts, skippedMode, skippedNoTexture, skippedNoUv, hasNoUvSkyPart ? 1 : 0);
        RendererLog(msg);
