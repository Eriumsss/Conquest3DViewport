...
// LevelReader.h is included for ZeroEngine::LotrHashString — same CRC32
// the dump filenames use. Texture lookup falls back to "0x{CRC:08X}.dds"
// in the same directory when the material-named file doesn't exist on
// disk. The 5696 dumps in GameFiles\textures cover every character in
// the game, so the asset browser preview works on any model regardless
// of which level BIN is currently loaded.
...
struct GlbPrimitive
{
    ...
    int meshIndex;
    int skinIndex;
    // Detected from mesh name: 0..4 = LOD level, 99 = collision/physics proxy.
    // Defaults to 0 (treat as primary) when the GLB doesn't follow the LOD
    // naming convention — that way unflagged models keep rendering exactly
    // like they did before this field existed.
    int lodLevel;
};

// Map a GLB mesh name to an LOD slot.
// The naming conventions across Maya / Blender / 3ds Max exporters are a zoo —
// match the obvious ones (case-insensitive substring) and bail to 0 otherwise.
static int ClassifyMeshNameToLod(const char* name)
{
    if (!name || !name[0]) return 0;
    char lower[256];
    int n = 0;
    while (name[n] && n < (int)sizeof(lower) - 1) {
        char c = name[n];
    ...
    if (strstr(lower, "collision") || strstr(lower, "_col") ||
        strstr(lower, "_phys")     || strstr(lower, "physics"))
        return 99;

    if (strstr(lower, "lod0") || strstr(lower, "_l0") || strstr(lower, "_high"))   return 0;
    if (strstr(lower, "lod1") || strstr(lower, "_l1") || strstr(lower, "_med"))    return 1;
    if (strstr(lower, "lod2") || strstr(lower, "_l2"))                             return 2;
    if (strstr(lower, "lod3") || strstr(lower, "_l3") || strstr(lower, "_low"))    return 3;
    if (strstr(lower, "lod4") || strstr(lower, "_l4") || strstr(lower, "_lowest")) return 4;
    return 0;
}
...
static int ParseGlbMeshPrimitives(const char* json, std::vector<GlbPrimitive>& out)
{
    ...
    while (m && arrEnd && m < arrEnd)
    {
        ...
        // Pull mesh name once and classify it — every primitive in this
        // mesh inherits the LOD slot. glTF puts the name on the mesh, not
        // on individual primitives, so this is the right granularity.
        char meshName[128];
        meshName[0] = '\0';
        ExtractStringValueRange(meshStart, meshEnd, "name", meshName, sizeof(meshName));
        const int meshLod = ClassifyMeshNameToLod(meshName);
        ...
        for (int i = 0; i < model->meshPartCount; i++)
        {
            ...
            if (part.diffuseName[0] != '\0')
            {
                ...
                part.diffuseTexture = loadDDSTexture(path,
                                                    part.diffuseLoadedPath,
                                                    sizeof(part.diffuseLoadedPath));
            }
...
bool GameModelLoader::loadMeshFromGLB(const char* glbPath, GameModel* model)
{
    ...
    for (int i = 0; i < model->meshPartCount; i++)
    {
        ...
        // Propagate LOD classification from the primitive's mesh name —
        // this is what lets the renderer filter LOD3/LOD4/collision pieces
        // and stop stacking them over the real LOD0 (the gray-overlay bug).
        model->meshParts[i].lodLevel = prim.lodLevel;

        // PROXY-MESH GUARD — Pandemic's character GLBs ship duplicates of
        // certain submeshes WITHOUT a TEXCOORD_0 channel (collision /
        // shadow / skin-bind proxies). The renderer still binds the
        // material's diffuse texture for those parts, but with no UVs
        // every vertex samples texel (0,0) of the atlas — a single gray
        // pixel — which paints a flat-shaded copy of the character on
        // top of the real textured mesh. The user spent two debug
        // rounds confused by a "ghost layer" that was just these
        // primitives.
        //
        // Promote any UV-less primitive into the collision-proxy LOD
        // slot (99). That slot is off by default in m_lodVisible[5], so
        // the parts vanish from the viewport unless the user explicitly
        // ticks "Col" in the Render Debug LOD row. Sky-UV generation
        // (skyUV=true) is the one legitimate UV-less case; leave those
        // alone so skyboxes still render.
        if (!model->meshParts[i].hasTexcoord0 &&
            !model->meshParts[i].generatedSkyUV) {
            model->meshParts[i].lodLevel = 99;
        }

        GameModelLog("[GameModelLoader] Mesh %d: %d verts, %d indices, lod=%d, hasUV=%d",
                     i, model->meshParts[i].vertexCount, model->meshParts[i].indexCount,
                     model->meshParts[i].lodLevel,
                     model->meshParts[i].hasTexcoord0 ? 1 : 0);
    }
...
// Extract the base name (no directory, no extension) from a path like
// "..\GameFiles\textures\CH_gdr_swd_all_01_HEAD_D.dds" → "CH_gdr_swd_all_01_HEAD_D".
// Used by the hex-CRC fallback to hash the material name.
static void ExtractTextureBaseName(const char* path, char* outName, size_t outSize)
{
    outName[0] = '\0';
    ...
    if (slash1 && slash1 >= fname) fname = slash1 + 1;
    if (slash2 && slash2 >= fname) fname = slash2 + 1;
    size_t n = strlen(fname);
    if (n >= 4 &&
        (fname[n-4] == '.') &&
        (fname[n-3] == 'd' || fname[n-3] == 'D') &&
        (fname[n-2] == 'd' || fname[n-2] == 'D') &&
        (fname[n-1] == 's' || fname[n-1] == 'S'))
    {
        n -= 4;
    }
    if (n >= outSize) n = outSize - 1;
    memcpy(outName, fname, n);
    outName[n] = '\0';
}

// Build "<dir>/0x{CRC:08X}.dds" given a path like
// "..\GameFiles\textures\Foo_D.dds" — the directory and extension stay,
// only the basename gets replaced with the hex hash. This is the naming
// convention every existing dump in GameFiles\textures uses (5696 files
// of the form 0x12345678.dds). For texture lookups, the master dump
// covers every character regardless of which level BIN is currently
// open — way better coverage than reading the loaded LevelReader.
static bool BuildHexCrcPath(const char* originalPath, char* outPath, size_t outSize)
{
    if (!originalPath || !originalPath[0]) return false;

    char baseName[256];
    ExtractTextureBaseName(originalPath, baseName, sizeof(baseName));
    if (!baseName[0]) return false;

    // Find where the basename starts in the original path so we can
    // preserve the directory prefix.
    const char* slash1 = strrchr(originalPath, '\\');
    const char* slash2 = strrchr(originalPath, '/');
    const char* sep = slash1;
    if (slash2 && (!sep || slash2 > sep)) sep = slash2;
    size_t dirLen = sep ? (size_t)(sep - originalPath + 1) : 0;

    uint32_t crc = ZeroEngine::LotrHashString(baseName);

    int rc = _snprintf_s(outPath, outSize, _TRUNCATE,
                         "%.*s0x%08X.dds", (int)dirLen, originalPath, crc);
    return rc > 0;
}

hkgTexture* GameModelLoader::loadDDSTexture(const char* texturePath,
                                            char* outEffectivePath,
                                            int outEffectivePathSize)
{
    if (outEffectivePath && outEffectivePathSize > 0) outEffectivePath[0] = '\0';
    ...
    // Pick the path to actually load from. If the material-name file
    // exists, use it (helmets, props that did get dumped under their
    // material names). Otherwise fall back to the hex-CRC convention
    // every dump file in GameFiles\textures uses: 0x{LotrHashString
    // (matName):08X}.dds. The 5696 dumps cover every character in the
    // game, including ones whose textures aren't in the currently
    // loaded level's BIN, so this works in the asset browser even
    // before any level is loaded.
    const char* effectivePath = texturePath;
    char hexPath[512];
    hexPath[0] = '\0';
    bool hexAttempted = false;
    bool hexExisted   = false;
    {
        FILE* f = fopen(texturePath, "rb");
        if (f) {
            fclose(f);
        } else {
            hexAttempted = BuildHexCrcPath(texturePath, hexPath, sizeof(hexPath));
            if (hexAttempted) {
                FILE* g = fopen(hexPath, "rb");
                if (g) {
                    fclose(g);
                    hexExisted = true;
                    effectivePath = hexPath;
                }
            }
        }
    }
    ...
    if (!texture->loadFromFile(effectivePath))
    {
        GameModelLog("[GameModelLoader] Failed to load texture: %s (hexAttempt=%s hexPath=%s exists=%d)",
                     texturePath,
                     hexAttempted ? "yes" : "no",
                     hexPath[0] ? hexPath : "<unbuilt>",
                     hexExisted ? 1 : 0);
    ...
    if (effectivePath != texturePath) {
        GameModelLog("[GameModelLoader] Loaded texture via hex-CRC: %s -> %s",
                     texturePath, effectivePath);
    } else {
        GameModelLog("[GameModelLoader] Loaded texture: %s", texturePath);
    }
    if (outEffectivePath && outEffectivePathSize > 0) {
        strncpy(outEffectivePath, effectivePath, outEffectivePathSize - 1);
        outEffectivePath[outEffectivePathSize - 1] = '\0';
    }
    return texture;
}
