...
inline hkReal hkaDefaultAnimationControl::easeIn(hkReal duration)
{
	...
	// Check for being truly in middle of easing out -- see JIRA 672
	const hkBool inTheMiddleOfEasingOut = ( m_easeStatus == EASING_OUT ) && ( m_easeT != 0.0f );
	m_easeT = inTheMiddleOfEasingOut ? ( 1.0f - m_easeT ) : 0.0f;

	m_easeInvDuration = (duration > HK_REAL_EPSILON) ? (1.0f / denominator) : HK_REAL_MAX;
	m_easeStatus = EASING_IN;
...
inline hkReal hkaDefaultAnimationControl::easeOut(hkReal duration)
{
	...
	// Check for being truly in middle of easing in -- see JIRA 672
	const hkBool inTheMiddleOfEasingIn = ( m_easeStatus == EASING_IN ) && ( m_easeT != 0.0f );
	m_easeT = inTheMiddleOfEasingIn ? ( 1.0f - m_easeT ) : 0.0f;

	m_easeInvDuration = (duration > HK_REAL_EPSILON) ? (1.0f / denominator) : HK_REAL_MAX;
	m_easeStatus = EASING_OUT;
