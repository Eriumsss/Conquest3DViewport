// Crc32Tables.cpp
// =============================================================================
// CRC-32/MPEG-2 tables and the goddamn Pandemic-CRC routine
// =============================================================================
// Written by: Eriumsss
//
// The level format hashes type names and field names with CRC-32/MPEG-2
// (NOT the reflected CRC-32 every internet tutorial assumes you mean
// when you say "CRC-32"). This was reverse-engineered from the engine
// at 0x0067e699 and locked in project_disassembly_findings.md - see
// that file before you touch anything in here.
//
// Vespucci needs it because:
//   - Schema/ZETypeRegistry hashes type-name strings to TypeId.
//   - Compat/RuleSet hashes event/action names to compare against
//     the level's gameObjOutputEvents[] which carry pre-hashed CRCs.
//   - Corpus/CorpusFormatV1 stamps a CRC over each shard for
//     atomic-load integrity check.
//
// Three tables here:
//   - kCrc32MpegTable: the 256-entry forward table for poly 0x04C11DB7,
//     init 0xFFFFFFFF, NOT reflected, NOT XORed at output.
//   - kCrc32MpegFinTable: same poly, but XORed-out at end (for the
//     places the engine post-processes).
//   - kCrc32IsoTable: standard CRC-32 (poly 0x04C11DB7 reflected,
//     init 0xFFFFFFFF, XOR-out 0xFFFFFFFF). For Corpus shard checksums
//     where we want a well-known algorithm any consumer can validate.
//
// Tables are precomputed at file scope. Generation code below is left
// in place under #if 0 so future-us can regenerate if the polynomials
// ever need to change (they will not, this is forensic preservation
// of a binary format we cannot rewrite).
// =============================================================================
...
// ── CRC-32/MPEG-2 table (poly 0x04C11DB7, NOT reflected) ──────────────
// Generated by: kCrc32MpegTable[i] = let crc = i << 24
//               for 8 bits: crc = (crc & 0x80000000) ? (crc << 1) ^ poly : (crc << 1)
const u32 kCrc32MpegTable[256] = {
...
// ── Pandemic / engine CRC routine ─────────────────────────────────────
// Init 0xFFFFFFFF, NO output XOR, NOT reflected. This is the routine
// that produced the field-name CRCs scattered through level.json.
u32 PandemicCrcBytes(const void* bytes, usize n) {
    const u8* p = (const u8*)bytes;
    u32 crc = 0xFFFFFFFFu;
    for (usize i = 0; i < n; ++i) {
        crc = (crc << 8) ^ kCrc32MpegTable[((crc >> 24) ^ p[i]) & 0xFFu];
    }
    return crc;
}

u32 PandemicCrcCStr(const char* s) {
    if (!s) return 0xFFFFFFFFu;
    usize n = 0;
    while (s[n]) ++n;
    return PandemicCrcBytes(s, n);
}

// ── ISO CRC-32 (reflected, XOR-out 0xFFFFFFFF) ────────────────────────
// Used for our own corpus-shard integrity stamping. The reflected
// table is generated lazily; we don't ship a 1KB constant for it
// because corpus-write is offline-only and we don't care about a
// 256-iteration cold-start cost. Implementation lives in Corpus/
// (CorpusWriter.cpp); no exports from here for ISO.
