...
/**
    ...
    // collision geometry, spawn points, the whole goddamn thing. The level data
...
public:
    ...
    // Cinematic camera override — when active, bypasses orbit camera
    void setCinematicOverride(bool active, float eyeX, float eyeY, float eyeZ,
                              float tgtX, float tgtY, float tgtZ, float fov) {
        m_cineOverride = active;
        if (active) {
            m_cameraTarget.set(tgtX, tgtY, tgtZ, 0.0f);
            // Compute orbit params from eye/target
            float dx = eyeX - tgtX, dy = eyeY - tgtY, dz = eyeZ - tgtZ;
            m_cameraDistance = sqrtf(dx*dx + dy*dy + dz*dz);
            if (m_cameraDistance < 0.01f) m_cameraDistance = 0.01f;
            m_cameraPitch = asinf(dy / m_cameraDistance);
            m_cameraYaw = atan2f(dx, dz);
            m_cameraFovDegrees = fov;
            if (m_camera) m_camera->setFOV(fov);
        }
    }
    bool isCinematicOverride() const { return m_cineOverride; }
...
public:
    ...
    // ---------------------------------------------------------------
    // GameModel LOD + render-mode debug toggles. Appended at the very
    // END of the class so the .obj layout stays stable for anything
    // built against an older header. Slots 0..4 are visual LODs (LOD0 =
    // highest detail), slot 5 holds the collision-proxy bucket (mesh
    // names containing "collision" / "_col" / "_phys" land here).
    //
    // Default state: LOD0 ON, everything else OFF. That alone fixes the
    // "extra gray low-poly layer over the real mesh" bug, because the
    // old code was happily drawing every LOD primitive on top of LOD0
    // and letting z-fighting decide who won.
    // ---------------------------------------------------------------
    bool  m_lodVisible[6];
    int   m_modelRenderMode;    // 0=Textured, 1=Untextured, 2=Wireframe, 3=FlatColor
    int   m_modelShaderMode;    // 0=Lit, 1=Unlit, 2=NormalsAsColor, 3=UVAsColor

    // ---------------------------------------------------------------
    // Per-pass kill switches for the stepAndRender pipeline. The
    // engine fires several render passes back to back — each can
    // potentially draw geometry that ends up layered over the main
    // character mesh. The "extra gray/black silhouette over the real
    // mesh" symptom turned out NOT to be LOD stacking (the GLBs don't
    // ship multi-LOD primitives), so these toggles let the user (and
    // us, debugging) isolate exactly which pass adds the ghost layer.
    //
    // Pass order in stepAndRender (line 2693+):
    //   passEnableSkeleton    -> renderSkeletonFromPose (bones+joints)
    //   passEnableMesh        -> renderMesh             (GameModel)
    //   passEnablePlayerChar  -> renderPlayerCharacter  (physics)
    //   passEnableDisplayWorld-> m_displayWorld->render (Havok auto)
    //   passEnableFx          -> m_effectManager->render(particles)
    //
    // displayWorld is the prime suspect for the silhouette overlay:
    // anything HavokToDisplayConverter registered there gets rendered
    // by Havok's own pipeline with shadows enabled, independent of
    // our renderMesh() path.
    // ---------------------------------------------------------------
    bool  m_passEnableSkeleton;
    bool  m_passEnableMesh;
    bool  m_passEnablePlayerChar;
    bool  m_passEnableDisplayWorld;
    bool  m_passEnableFx;

    // Which "shader path" the GameModel render uses. Right now only
    // path 0 (current Havok fixed-function via hkgDisplayContext
    // immediate mode) is fully wired. The other slots reserve room
    // for routing the model through the game's compiled shaders
    // (GameShaderCache lookup table — same path LevelScene uses)
    // or via the Havok displayWorld pipeline.
    //   0 = Havok Fixed-Function (current renderMesh path)
    //   1 = Havok DisplayWorld   (let Havok auto-render via hkgDisplayObject)
    //   2 = Game Shaders         (GameShaderCache compiled D3D9 shaders)
    //   3 = Unlit Debug          (force flat-white, no lighting)
    int   m_gameModelShaderPath;

    // For path 2 — name of the game shader (PS) the user picked from
    // the GameShaderCache. Empty == use the cache's default for the
    // material. Stored as a fixed-size buffer so we don't drag a
    // std::string into a layout-sensitive class.
    char  m_gameModelShaderPSName[96];

    // ---------------------------------------------------------------
    // Diagnostic instrumentation — turns "there's a black layer on the
    // character" into "pass X draws N triangles colored Y at position
    // Z." Pure debug; everything here is zero-cost when the toggles are
    // off. Stays at the END of the class for .obj layout stability.
    // ---------------------------------------------------------------

    // Per-pass vertex-color tint. When on, each render pass paints its
    // geometry with a distinct color so the user can spot which pass
    // owns the silhouette overlay. Opaque=red, transparent=blue,
    // skeleton=yellow, playerChar=magenta, displayWorld bracketed by
    // setStateRGBA(green) if the Havok context exposes a global tint.
    bool  m_tintPasses;

    // Per-frame stat counters bumped by each render function. Index
    // mapping matches the enum below — same order in the UI table.
    struct PassStats {
        int drawCalls;
        int triangles;
        int meshPartsDrawn;
    };
    enum { STAT_MESH = 0, STAT_SKELETON, STAT_PLAYERCHAR,
           STAT_DISPLAYWORLD, STAT_LEVELSCENE, STAT_FX, STAT_COUNT };
    PassStats m_passStats[6];

    // Cached count of objects sitting in m_displayWorld. Snapshotted
    // once per frame at the top of stepAndRender so the UI can warn
    // about stale registrations even when the displayWorld pass is
    // gated off (those objects don't render but they still exist).
    int   m_displayWorldObjectCountCached;

    // Force-single-pass debug toggle: 0=both passes (default), 1=opaque
    // only, 2=transparent only. Lets the user localize whether the
    // silhouette belongs to the opaque or the transparent half of
    // renderMesh.
    int   m_forcePassOnly;

    // Per-meshPart visibility override. The user clicks [Hide] in the
    // mesh-part inspector and whichever part owns the silhouette gets
    // ruled out one at a time. 64 slots is way more than any single
    // character ever needs (typical GLB ships 2–8 parts). Index out
    // of range == always visible.
    bool  m_partVisibleOverride[64];

    // Frame-dump request: host sets it, renderer writes
    // render_debug_dump.log at the end of stepAndRender and clears
    // the flag. One-shot so we don't spam the disk.
    bool  m_dumpRequested;

public:
    void setTintPasses(bool v)        { m_tintPasses = v; }
    bool getTintPasses() const        { return m_tintPasses; }
    void setForcePassOnly(int p) {
        if (p < 0) p = 0; else if (p > 2) p = 2;
        m_forcePassOnly = p;
    }
    int  getForcePassOnly() const     { return m_forcePassOnly; }
    void setPartVisibleOverride(int idx, bool v) {
        if (idx < 0 || idx >= 64) return;
        m_partVisibleOverride[idx] = v;
    }
    bool getPartVisibleOverride(int idx) const {
        if (idx < 0 || idx >= 64) return true;
        return m_partVisibleOverride[idx];
    }
    void requestFrameDump()           { m_dumpRequested = true; }
    const PassStats& getPassStats(int idx) const {
        static const PassStats s_empty = { 0, 0, 0 };
        if (idx < 0 || idx >= STAT_COUNT) return s_empty;
        return m_passStats[idx];
    }
    int  getDisplayWorldObjectCount() const {
        return m_displayWorldObjectCountCached;
    }

    // Loaded model access for the Render Debug meshPart inspector.
    // Returns the live pointer — host caller must NOT delete or
    // mutate; the renderer owns lifetime.
    struct GameModel* getGameModel() const { return m_gameModel; }

public:
    void setLodVisible(int slot, bool v) {
        if (slot < 0 || slot > 5) return;
        m_lodVisible[slot] = v;
    }
    bool isLodVisible(int slot) const {
        if (slot < 0 || slot > 5) return true;
        return m_lodVisible[slot];
    }
    void setModelRenderMode(int m) {
        if (m < 0) m = 0; else if (m > 3) m = 3;
        m_modelRenderMode = m;
    }
    int  getModelRenderMode() const { return m_modelRenderMode; }
    void setModelShaderMode(int m) {
        if (m < 0) m = 0; else if (m > 3) m = 3;
        m_modelShaderMode = m;
    }
    int  getModelShaderMode() const { return m_modelShaderMode; }

    void setPassEnableSkeleton(bool v)     { m_passEnableSkeleton = v; }
    void setPassEnableMesh(bool v)         { m_passEnableMesh = v; }
    void setPassEnablePlayerChar(bool v)   { m_passEnablePlayerChar = v; }
    void setPassEnableDisplayWorld(bool v) { m_passEnableDisplayWorld = v; }
    void setPassEnableFx(bool v)           { m_passEnableFx = v; }
    bool getPassEnableSkeleton()     const { return m_passEnableSkeleton; }
    bool getPassEnableMesh()         const { return m_passEnableMesh; }
    bool getPassEnablePlayerChar()   const { return m_passEnablePlayerChar; }
    bool getPassEnableDisplayWorld() const { return m_passEnableDisplayWorld; }
    bool getPassEnableFx()           const { return m_passEnableFx; }

    void setGameModelShaderPath(int p) {
        if (p < 0) p = 0; else if (p > 3) p = 3;
        m_gameModelShaderPath = p;
    }
    int  getGameModelShaderPath() const { return m_gameModelShaderPath; }
    void setGameModelShaderPSName(const char* name) {
        if (!name) { m_gameModelShaderPSName[0] = '\0'; return; }
        strncpy(m_gameModelShaderPSName, name, sizeof(m_gameModelShaderPSName) - 1);
        m_gameModelShaderPSName[sizeof(m_gameModelShaderPSName) - 1] = '\0';
    }
    const char* getGameModelShaderPSName() const { return m_gameModelShaderPSName; }
